...
import { ReciveMessage, ReciveMessagePermission } from '@prisma/client'
import TextInputSearch from '../FormInputs/TextInputSearch'
import { reciveMessageColumn } from '@/app/(dummy)/data'

export default function ReciveMessagesData({
  data,
  permission,
}: {
  data: ReciveMessage[]
  ...
  const newData = useMemo(() => {
    return (
      data?.length > 0 &&
      data?.filter((item: ReciveMessage) => {
        const searchValue =
          search === '' ||
  ...
  return (
    <div>
      ...
      <DataTable
        data={newData}
        columns={reciveMessageColumn}
        onDeleted={permission?.reciveMessageDelete}
        onView={permission?.reciveMessageView}
        linkToView="/dashboard/recive-messages/view/"
        resourceName="Recive Message"
      />
    </div>
  )
}

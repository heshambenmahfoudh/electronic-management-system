...
import { ArchiveMessagePermission, ReciveArchive } from '@prisma/client'
import TextInputSearch from '../FormInputs/TextInputSearch'
import { archiveReciveMessageColumn } from '@/app/(dummy)/data'
...
export default function ReciveMessagesArchiveData({
  data,
  permission,
  officeId,
}: {
  ...
  const newData = useMemo(() => {
    return (
      data?.length > 0 &&
      data?.filter((item: ReciveArchive) => {
        const searchValue =
          !search ||
  ...
  const initialData =
    newData &&
    newData?.filter((item: ReciveArchive) => item?.id === numberId)?.[0]
  ...
  return (
    <div>
      <FixedHeader
        textValue="Archive Recive Messages"
      ...
      <DataTable
        data={newData}
        columns={archiveReciveMessageColumn}
        onView={permission?.ArchiveReciveMessageView}
        onDeleted={permission?.ArchiveReciveMessageDelete}
        resourceName="Archive Recive Message"
        handleClickGetId={handleClickGetId}
      />
      {openViewModel && <VeiwMessageForm initialData={initialData} />}
    </div>
  )
}

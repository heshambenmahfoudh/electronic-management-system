...
import {
  getSettingPermissionById,
  updateSettingPermissionById,
} from '@/actions/permissions/settingPermissions'
...
export default function SettingPermissionForm() {
  ...
  useEffect(() => {
    if (permissionData) {
      Object.entries(permissionData?.data as SettingPermission)?.forEach(
        ([key, value]) => {
          setValue(key as keyof SettingPermission, value as boolean)
        },
      )
    } else {
      reset()
  ...
  async function onSubmit(data: SettingPermission) {
    setIsLoading(true)
    const updatedPermission = await updateSettingPermissionById(data)
    if (updatedPermission?.status === 200) {
      reset()
      setIsLoading(false)
      query.invalidateQueries({ queryKey: ['settingPermission'] })
      toast.success('User setting permission updated successfully')
    } else {
      setIsLoading(false)

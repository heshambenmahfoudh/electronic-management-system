...
import {
  getBackupPermissionById,
  updateBackupPermissionById,
} from '@/actions/permissions/backupPermissions'
...
export default function BackupPermissionForm() {
  ...
  useEffect(() => {
    if (permissionData) {
      Object.entries(permissionData?.data as BackUpPermission)?.forEach(
        ([key, value]) => {
          setValue(key as keyof BackUpPermission, value as boolean)
        },
      )
    } else {
      reset()
  ...
  async function onSubmit(data: BackUpPermission) {
    setIsLoading(true)
    const updatedPermission = await updateBackupPermissionById(data)
    if (updatedPermission?.status === 200) {
      reset()
      setIsLoading(false)
      query.invalidateQueries({ queryKey: ['backupPermission'] })
      toast.success('User back up permission updated successfully')
    } else {
      setIsLoading(false)
      ...
      <form onSubmit={handleSubmit(onSubmit)}>
        <div className="grid md:grid-cols-3 grid-cols-1  gap-2 mt-6">
          <div className="w-fit border-1 border-blue-300 p-3 pt-2  rounded-lg">
            <h2>Take Backup Copy</h2>
            <div className="grid grid-cols-4 gap-2 mt-2">
              <CheckBoxInput
                label="Display"
                name="takeBackupDisplay"
                register={register}
                className={className}
              />
              <CheckBoxInput
                label="Create"
                name="takeBackupCreate"
                register={register}
                className={className}
              />
              <CheckBoxInput

...
export default function ArchiveDelayMessageForm({
  initialData,
  permission,
}: {
  initialData: any
  ...
  async function onSubmit(data: TypeArchiveAndDelayProps) {
    ...
    data.typeOperation = 'delay'
    try {
      const response = await createNewReciveArchive(data)
      if (response?.status === 200) {
        reset()
        setIsLoading(false)
        router.push('/dashboard/recive-messages/delayed-messages')
        toast.success(`Archive message successfully`)
      } else {
        setIsLoading(false)
  ...
  return (
    <div>
      <FormHeader
        titleValue="Delay message details"
        linkUrl="/dashboard/recive-messages/delayed-messages"
      />
      <form
      ...
      >
        ...
        {permission?.delayMessageArchive ? (
